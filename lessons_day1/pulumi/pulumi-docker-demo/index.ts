...
class WebAppStack extends pulumi.ComponentResource {
  ...
  constructor(
    ...
    // Pull Redis image
    const redisImage = new docker.RemoteImage(
      `${name}-redis-image`,
      {
        name: "redis:7-alpine",
        keepLocally: true,
    ...
    this.redisContainer = new docker.Container(
      `${name}-redis`,
      {
        name: `redis-${name}`,
...
http {
    ...
    this.nginxContainer = new docker.Container(
      `${name}-nginx`,
      {
        name: `nginx-${name}`,
        ...
        envs: [`REDIS_HOST=redis`, `STACK_NAME=${name}`, `PORT=${port}`],
        uploads: [
          {
            content: nginxConfig,
